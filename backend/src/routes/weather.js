...
const VISUAL_CROSSING_API_BASE_URL = 'https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline';
...
//     const geoResponse = await axios.get(`${WEATHER_API_BASE_URL}/weather`, {
//       params: {
//         q: city,
//         appid: WEATHER_API_KEY,
//       },
...
//     // Get the 7-day forecast using the One Call API
//     const forecastResponse = await axios.get(`${WEATHER_API_BASE_URL}/onecall`, {
//       params: {
//         lat: coord.lat,
//         lon: coord.lon,
...
//     // Get the 7-day forecast using the One Call API
//     const forecastResponse = await axios.get(`${WEATHER_API_BASE_URL_ONCALL}`, {
//       params: {
//         lat: coord.lat,
//         lon: coord.lon,
//         exclude: 'current,minutely,hourly,alerts',
...
//       res.status(500).json({ error: 'No response received from the weather API' });
//     } else {
...
router.get('/forecasts', async (req, res) => {
  try {
    ...
    const forecastResponse = await axios.get(`${WEATHER_API_BASE_URL}/forecast`, {
      params: {
        q: city,
        appid: WEATHER_API_KEY,
        units: 'metric',
...
router.get('/historical', async (req, res) => {
  try {
    ...
    const historicalResponse = await axios.get(`${VISUAL_CROSSING_API_BASE_URL}/${city}/${formattedStartDate}/${formattedEndDate}`, {
      params: {
        unitGroup: 'metric',
    ...
    console.log(`API Request URL: ${VISUAL_CROSSING_API_BASE_URL}/${city}/${formattedStartDate}/${formattedEndDate}`);
    ...
    const transformedData = {
      city: {
        ...
        country: historicalResponse.data.resolvedAddress.split(',').pop().trim(),
      },
